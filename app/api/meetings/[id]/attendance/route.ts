...
export async function GET(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const attendedShares = attendanceList.reduce((sum, a) => sum + Number(a.shareholder.shareValue), 0);
    ...
    return NextResponse.json({ error: err.message ?? 'Failed to get attendance' }, { status: 500 });
  }
}
...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const shareholderIds: string[] = parsed.shareholderIds ?? (parsed.shareholderId ? [parsed.shareholderId] : []);
    if (shareholderIds.length === 0) {
      return NextResponse.json({ error: 'No shareholderIds provided' }, { status: 400 });
    }

    const meeting = await prisma.meeting.findUnique({ where: { id }, include: { attendances: true } });
    if (!meeting) return NextResponse.json({ error: 'Meeting not found' }, { status: 404 });
    if (meeting.status === 'CLOSED') {
      return NextResponse.json({ error: 'Cannot modify attendance for closed meeting' }, { status: 400 });
    }
    ...
    let representativeId: string | null = parsed.representativeId?.trim() || null;
    ...
    if (!representativeId && parsed.representativeShareholderId) {
      const sh = await prisma.shareholder.findUnique({ where: { id: parsed.representativeShareholderId } });
      if (!sh) return NextResponse.json({ error: 'Representative shareholder not found' }, { status: 404 });

      // Try to find existing Representative that uses this shareholder
      const existingRep = await prisma.representative.findFirst({ where: { shareholderId: parsed.representativeShareholderId } });
      if (existingRep) {
    ...
    if (representativeId && parsed.representativeName) {
      const existing = await prisma.representative.findUnique({ where: { id: representativeId } });
      if (!existing) {
    ...
    for (const shId of shareholderIds) {
      ...
      createdAttendances.push({
        ...
        representedByName: created.representedBy?.name ?? created.representativeName ?? null,
    ...
    return NextResponse.json({ ok: true, created: createdAttendances, skipped });
  } catch (err: any) {
    if (err?.issues) return NextResponse.json({ error: err.issues }, { status: 400 });
    return NextResponse.json({ error: err.message ?? 'Failed to add attendance' }, { status: 400 });
  }
}
