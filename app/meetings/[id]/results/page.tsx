...
if (!meeting) return <div className="p-8 text-center">Loading results...</div>;
...
<CardTitle className="text-xl font-semibold">Voting Results</CardTitle>
<Badge variant={meeting.status === 'CLOSED' ? 'secondary' : 'outline'}>
{meeting.status === 'CLOSED' ? 'Closed' : 'In Progress'}
</Badge>
</CardHeader>
<CardContent>
{loading && <p className="text-sm text-gray-500">Refreshing results...</p>}


{results.length === 0 ? (
<p className="text-center text-gray-500 py-6">No votes recorded yet.</p>
) : (
<div className="space-y-4">
{results.map((r, i) => {
const percentage = totalWeight > 0 ? (r.totalWeight / totalWeight) * 100 : 0;
return (
<div
key={r.nomineeId}
