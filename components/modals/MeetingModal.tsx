...
export default function MeetingModal({ isOpen, onClose, mode, initialData, onSuccess }: MeetingModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (!quorum || isNaN(Number(quorum))) frontendErrors.push('Valid quorum is required');
    ...
    setLoading(true);
    try {
      const url = mode === 'add' ? '/api/meetings' : `/api/meetings/${initialData.id}`;
      const method = mode === 'add' ? 'POST' : 'PUT';

      const res = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title, date, location, quorum, status }),
      });
      ...
      if (!res.ok) {
        ...
        if (data.error && Array.isArray(data.error)) {
          toast.error(data.error.map((e: any) => e.message || JSON.stringify(e)));
        } else if (data.error) {
        ...
        <div className="space-y-4 mt-2">
          ...
          {mode === 'edit' && (
            <div>
              ...
              <select value={status} onChange={e => setStatus(e.target.value)} className="border rounded p-1 w-full">
